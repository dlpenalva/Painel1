...
class TestCascaXlsFirst(unittest.TestCase):
    ...
    def test_documentos_fica_enxuto_antes_do_upload_e_preserva_o_motor(self):
        ...
        self.assertNotIn("Baixar arquivo de trabalho", DOCUMENTOS)
        self.assertNotIn("Baixar Arquivo Coleta Oficial", DOCUMENTOS)
        self.assertNotIn("_coleta_oficial_cacheada", DOCUMENTOS)
        self.assertNotIn("assinatura_template_coleta", DOCUMENTOS)
        self.assertIn("Enviar Arquivo Coleta Oficial preenchido", DOCUMENTOS)
    ...
    def test_central_e_hub_compacto_dos_seis_documentos_oficiais(self):
        ...
        self.assertIn("transform:translateY(-.04rem)", CENTRAL)
        self.assertIn("height:18rem", CENTRAL)
        self.assertIn("height:8.05rem", CENTRAL)
        self.assertIn("-webkit-line-clamp:3", CENTRAL)
        self.assertIn("bottom:15px", CENTRAL)
        self.assertIn("position:absolute", CENTRAL)

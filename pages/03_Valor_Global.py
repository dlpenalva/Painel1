...
def gerar_excel_valores_unitarios_por_ciclo(df_valores, ciclos):
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        for row_idx, (_, row) in enumerate(df_export.iterrows(), start=1):
            ...
            for col, col_idx in colunas.items():
                ...
                if col in ["Valor unitário", "Total R$"]:
                    ...
                    ws.write_number(row_idx, col_idx, numero_seguro(value, 0.0), fmt_num_red if precluso else fmt_num)
                elif col == "Ciclo precluso":
                    ws.write(row_idx, col_idx, "Sim" if precluso else "Não", fmt_text_red if precluso else fmt_text)
                else:
                    ws.write(row_idx, col_idx, value, fmt_text_red if precluso else fmt_text)
        ...
        for ciclo in ciclos_ordenados:
            ws_m.merge_range(0, col_atual, 0, col_atual + 2, ciclo, fmt_header)
            ws_m.write(1, col_atual, "Valor unitário", fmt_subheader)
            ws_m.write(1, col_atual + 1, "Quantidade", fmt_subheader)
            ws_m.write(1, col_atual + 2, "Total R$", fmt_subheader)
            col_atual += 3
        ...
        for row_idx, item in enumerate(itens_ordenados, start=2):
            ...
            for ciclo in ciclos_ordenados:
                ...
                ws_m.write_number(row_idx, col_atual, vu, fmt_money_red if precluso else fmt_money)
                ws_m.write_number(row_idx, col_atual + 1, qtd, fmt_num_red if precluso else fmt_num)
                ws_m.write_number(row_idx, col_atual + 2, total, fmt_money_red if precluso else fmt_money)
                col_atual += 3
...
def inferir_ciclo_por_data(data_aditivo, ciclos):
    ...
    Regra obrigatória:
    - se Data_Aditivo >= Início_Ciclo_N e Data_Aditivo <= Fim_Ciclo_N, retorna Ciclo_N;
    - se Data_Aditivo for anterior ao primeiro ciclo, retorna C0;
    - se a data estiver vazia/inválida ou sem correspondência, retorna "Fora de Ciclo".
    """
    ...
    for marco in marcos:
        if data >= marco["inicio"] and data <= marco["fim"]:
            return marco["ciclo"]

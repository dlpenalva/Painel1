...
if 'dados_admissibilidade' not in st.session_state:
    # Valores padrão para teste isolado
...
# --- 1. PARÂMETROS IMPORTADOS ---
st.header("1. Parâmetros do Reajuste")
st.info("Dados importados automaticamente da Etapa de Admissibilidade.")
...
def gerar_planilha_fiscal():
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        for row in range(3, 13):
            ws_itens.write(row, 0, row-2, fmt_numero) # Coluna A: Número
            ws_itens.write(row, 1, 0, fmt_numero)    # Coluna B: Qtd
            ws_itens.write(row, 2, 0, fmt_moeda)     # Coluna C: R$
            ws_itens.write_formula(row, 3, f'=B{row+1}*C{row+1}', fmt_moeda) # Coluna D: R$
        ...
        for row in range(3, 15):
            ...
            ws_retro.write(row, 1, 0, fmt_moeda) # Coluna B: R$
...
st.download_button(label="📥 Gerar Planilha para o Fiscal", data=gerar_planilha_fiscal(), file_name="Coleta_Global.xlsx")

st.divider()

# --- 3. PROCESSAMENTO ---
tabs = st.tabs(["📦 Processar Retorno", "⚖️ Comparativo"])

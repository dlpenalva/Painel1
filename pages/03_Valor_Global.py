...
def mapa_colunas_remanescentes_por_ciclo(colunas_remanescentes):
    mapa = {}
    for col in colunas_remanescentes or []:
        ciclo = ciclo_da_coluna_remanescente(col, fallback="")
        if ciclo:
            mapa[normalizar_ciclo(ciclo)] = col
    return mapa


def calcular_fatores_unitarios_por_ciclo(ciclos):
    """Monta a sequência de fatores aplicáveis aos valores unitários por ciclo.

    Ciclos preclusos ou sem reajuste aplicável repetem o fator anterior.
    """
    fatores = []
    fator_corrente = 1.0
    ...
    for _, row in df.iterrows():
        ...
        if not ciclo or ciclo == "C0":
            continue
        situacao = row.get("Situação", "")
        bloqueado = situacao_bloqueia_reajuste(situacao)
        if bloqueado:
            fator_aplicado = fator_corrente
        else:
            fator_lido = row.get("Fator acumulado efetivo", row.get("Fator acumulado", None))
            try:
                fator_lido = float(fator_lido)
            except Exception:
                fator_lido = fator_corrente
            if fator_lido <= 0:
                fator_lido = fator_corrente
            fator_corrente = fator_lido
            fator_aplicado = fator_corrente
        fatores.append({
            "Ciclo": ciclo,
            "Situação": situacao,
            "Ciclo precluso/bloqueado": bool(bloqueado),
            "Fator acumulado aplicado": float(fator_aplicado),
            "Reajuste acumulado aplicado": float(fator_aplicado) - 1,
        })
    return fatores


def montar_valores_unitarios_por_ciclo(itens, colunas_remanescentes, ciclos):
    if itens is None or itens.empty:
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()

    mapa_rem = mapa_colunas_remanescentes_por_ciclo(colunas_remanescentes)
    fatores_ciclo = calcular_fatores_unitarios_por_ciclo(ciclos)

    linhas = []
    for _, item in itens.iterrows():
        item_id = item.get("Item", "")
        vu_original = float(item.get("Valor unitário original", 0.0) or 0.0)
        qtd_c0 = float(item.get("Quantidade contratada", 0.0) or 0.0)
        linhas.append({
            "Item": item_id,
            "Ciclo": "C0",
            "Valor unitário": vu_original,
            "Quantidade": qtd_c0,
            "Total R$": vu_original * qtd_c0,
            "Situação do ciclo": "Base original",
            "Ciclo precluso/bloqueado": False,
        })
        for info in fatores_ciclo:
            ciclo = info["Ciclo"]
            col_qtd = mapa_rem.get(ciclo)
            qtd = float(item.get(col_qtd, 0.0) or 0.0) if col_qtd else 0.0
            fator = float(info.get("Fator acumulado aplicado", 1.0) or 1.0)
            ...
            linhas.append({
                ...
                "Situação do ciclo": info.get("Situação", ""),
                "Ciclo precluso/bloqueado": bool(info.get("Ciclo precluso/bloqueado", False)),
            })

    df_long = pd.DataFrame(linhas)
    df_ciclos = pd.DataFrame([{
        "Ciclo": "C0",
        "Situação": "Base original",
        "Fator acumulado aplicado": 1.0,
        "Reajuste acumulado aplicado": 0.0,
        "Ciclo precluso/bloqueado": False,
    }] + fatores_ciclo)
    df_matriz = montar_matriz_valores_unitarios(df_long)
    return df_long, df_matriz, df_ciclos


def montar_matriz_valores_unitarios(df_long):
    if df_long is None or df_long.empty:
        return pd.DataFrame()
    ciclos_ordenados = sorted(df_long["Ciclo"].dropna().unique(), key=numero_ciclo)
    linhas = []
    for item, grupo in df_long.groupby("Item", dropna=False):
        linha = {"Item": item}
        for ciclo in ciclos_ordenados:
            rec = grupo[grupo["Ciclo"] == ciclo]
            if rec.empty:
                linha[f"{ciclo} - Valor unitário"] = 0.0
                linha[f"{ciclo} - Quantidade"] = 0.0
                linha[f"{ciclo} - Total R$"] = 0.0
                linha[f"{ciclo} - Precluso/Bloqueado"] = False
            else:
                r = rec.iloc[0]
                linha[f"{ciclo} - Valor unitário"] = float(r.get("Valor unitário", 0.0) or 0.0)
                linha[f"{ciclo} - Quantidade"] = float(r.get("Quantidade", 0.0) or 0.0)
                linha[f"{ciclo} - Total R$"] = float(r.get("Total R$", 0.0) or 0.0)
                linha[f"{ciclo} - Precluso/Bloqueado"] = bool(r.get("Ciclo precluso/bloqueado", False))
        linhas.append(linha)
    return pd.DataFrame(linhas)


def gerar_excel_valores_unitarios(df_long, df_matriz, df_ciclos):
    output = BytesIO()
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        df_long_export = df_long.drop(columns=["Ciclo precluso/bloqueado"], errors="ignore")
        df_matriz_export = df_matriz[[c for c in df_matriz.columns if "Precluso/Bloqueado" not in str(c)]].copy()
        df_long_export.to_excel(writer, sheet_name="VALORES_POR_CICLO", index=False)
        df_matriz_export.to_excel(writer, sheet_name="MATRIZ_VALORES_CICLO", index=False)
        ...
        qty_fmt = wb.add_format({"num_format": '#,##0.00', "border": 1})
        txt_fmt = wb.add_format({"border": 1})
        red_money_fmt = wb.add_format({"num_format": 'R$ #,##0.00', "font_color": "#C00000", "border": 1})
        red_qty_fmt = wb.add_format({"num_format": '#,##0.00', "font_color": "#C00000", "border": 1})
        red_txt_fmt = wb.add_format({"font_color": "#C00000", "border": 1})
        pct_fmt = wb.add_format({"num_format": '0.00%', "border": 1})
        factor_fmt = wb.add_format({"num_format": '0.0000', "border": 1})

        ws = writer.sheets["VALORES_POR_CICLO"]
        for col_idx, col in enumerate(df_long_export.columns):
            ws.write(0, col_idx, col, header_fmt)
            ws.set_column(col_idx, col_idx, 20)
        for row_idx, (_, row) in enumerate(df_long.iterrows(), start=1):
            precluso = bool(row.get("Ciclo precluso/bloqueado", False))
            for col_idx, col in enumerate(df_long_export.columns):
                val = row.get(col, "")
                if col in ["Valor unitário", "Total R$"]:
                    ws.write_number(row_idx, col_idx, float(val or 0.0), red_money_fmt if precluso else money_fmt)
                elif col == "Quantidade":
                    ws.write_number(row_idx, col_idx, float(val or 0.0), red_qty_fmt if precluso else qty_fmt)
                else:
                    ws.write(row_idx, col_idx, val, red_txt_fmt if precluso else txt_fmt)

        ws = writer.sheets["MATRIZ_VALORES_CICLO"]
        for col_idx, col in enumerate(df_matriz_export.columns):
            ws.write(0, col_idx, col, header_fmt)
            ws.set_column(col_idx, col_idx, 20)
        for row_idx, (_, row) in enumerate(df_matriz.iterrows(), start=1):
            for col_idx, col in enumerate(df_matriz_export.columns):
                val = row.get(col, "")
                precluso_col = str(col).replace(" - Valor unitário", " - Precluso/Bloqueado").replace(" - Quantidade", " - Precluso/Bloqueado").replace(" - Total R$", " - Precluso/Bloqueado")
                precluso = bool(row.get(precluso_col, False))
                if "Valor unitário" in str(col) or "Total R$" in str(col):
                    ws.write_number(row_idx, col_idx, float(val or 0.0), red_money_fmt if precluso else money_fmt)
                elif "Quantidade" in str(col):
                    ws.write_number(row_idx, col_idx, float(val or 0.0), red_qty_fmt if precluso else qty_fmt)
                else:
                    ws.write(row_idx, col_idx, val, red_txt_fmt if precluso else txt_fmt)

        ws = writer.sheets["CICLOS_CONSIDERADOS"]
        for col_idx, col in enumerate(df_ciclos.columns):
            ws.write(0, col_idx, col, header_fmt)
            ws.set_column(col_idx, col_idx, 24)
        for row_idx, (_, row) in enumerate(df_ciclos.iterrows(), start=1):
            precluso = bool(row.get("Ciclo precluso/bloqueado", False))
            for col_idx, col in enumerate(df_ciclos.columns):
                val = row.get(col, "")
                if "Fator" in str(col):
                    ws.write_number(row_idx, col_idx, float(val or 0.0), factor_fmt)
                elif "Reajuste" in str(col):
                    ws.write_number(row_idx, col_idx, float(val or 0.0), pct_fmt)
                else:
                    ws.write(row_idx, col_idx, val, red_txt_fmt if precluso else txt_fmt)
    output.seek(0)
    return output
...
def processar_arquivo_coleta(bytes_arquivo):
    ...
    df_valores_unitarios, df_matriz_valores_unitarios, df_ciclos_valores_unitarios = montar_valores_unitarios_por_ciclo(
        itens, colunas_remanescentes, ciclos
    )
...
if resultado:
    ...
    col11.metric("Reajuste acumulado", percentual(float(resultado.get("fator_acumulado", 1.0)) - 1, 2))
    ...
    with tab5:
        ...
        st.caption(
            "C0 utiliza a quantidade contratada original. Os ciclos seguintes utilizam o remanescente informado no início de cada ciclo. "
            "Quando o ciclo estiver precluso ou sem reajuste aplicável, o valor unitário repete o ciclo anterior."
        )
        df_valores = resultado.get("df_valores_unitarios", pd.DataFrame())
        df_matriz = resultado.get("df_matriz_valores_unitarios", pd.DataFrame())
        df_ciclos_val = resultado.get("df_ciclos_valores_unitarios", pd.DataFrame())

        if df_valores.empty:
            st.info("Não há dados suficientes para montar a tabela de valores unitários por ciclo.")
        else:
            st.markdown("#### Matriz de comparação por item")
            df_matriz_vis = formatar_dataframe_moeda(
                df_matriz[[c for c in df_matriz.columns if "Precluso/Bloqueado" not in str(c)]],
            ...
            with st.expander("Ver tabela detalhada por item e ciclo"):
                df_valores_vis = formatar_dataframe_moeda(
                    df_valores.drop(columns=["Ciclo precluso/bloqueado"], errors="ignore"),
            ...
            st.download_button(
                ...
                data=gerar_excel_valores_unitarios(df_valores, df_matriz, df_ciclos_val),

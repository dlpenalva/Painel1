...
def sinal_status(valor):
    mapa = {"Validado": "🟢", "Pendente": "🔴", "Em conferência": "🟡", "Não se aplica": "⚪"}
...
def checklist_inicial():
    return pd.DataFrame([
        {"Grupo": grupo, "Item": item, "Status": "Pendente", "Criticidade": criticidade, "Observação": ""}
...
def gerar_excel_checklist(df, resumo):
    ...
    with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
        ...
        last_row = len(df)
        ws.conditional_format(1, status_col, last_row, status_col, {"type": "text", "criteria": "containing", "value": "Validado", "format": fmt_validado})
        ws.conditional_format(1, status_col, last_row, status_col, {"type": "text", "criteria": "containing", "value": "Pendente", "format": fmt_pendente})
        ws.conditional_format(1, status_col, last_row, status_col, {"type": "text", "criteria": "containing", "value": "Em conferência", "format": fmt_conferencia})
        ws.conditional_format(1, status_col, last_row, status_col, {"type": "text", "criteria": "containing", "value": "Não se aplica", "format": fmt_na})
...
with st.expander("Restaurar checklist a partir de XLSX", expanded=False):
    arquivo_checklist = st.file_uploader("Enviar Checklist_Processual.xlsx salvo anteriormente", type=["xlsx"], key="upload_checklist_processual")
    if arquivo_checklist is not None:
        if st.button("Importar status do checklist enviado", type="primary"):
            try:
                ...
                st.success("Checklist importado com sucesso.")
                st.rerun()
            except Exception as exc:
...
st.download_button("Baixar Checklist Processual em XLSX", data=excel,

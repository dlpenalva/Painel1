...
def tabela_inicial():
    df = pd.DataFrame([{
        "Seq.": 1, "Data": pd.NaT, "Tipo do evento": "",
        "Descrição / instrumento": "", "% sobre base legal": "",
        "Valor informado (R$)": "", "Observação": "",
    }])
...
def normalizar_linhas_eventos(df):
    ...
    if "Data" in df.columns:
        df["Data"] = pd.to_datetime(df["Data"], errors="coerce")
    colunas_texto = ["Tipo do evento", "Descrição / instrumento", "% sobre base legal", "Valor informado (R$)", "Observação"]
...
def calcular_eventos(df, base_legal, limite_acrescimos):
    ...
    for _, row in df.iterrows():
        ...
        linhas.append({
            "Seq.": row.get("Seq.", ""), "Data": row.get("Data", ""),
            "Tipo do evento": tipo, "Descrição / instrumento": descricao,
            "% sobre base legal": percentual, "Valor informado (R$)": valor_num if abs(valor_num) > 0 else "",
            "Valor do evento (R$)": round(valor_evento, 2), "Tratamento": tratamento,
            "Conta acréscimos (R$)": round(conta_acrescimos, 2), "Conta supressões (R$)": round(conta_supressoes, 2),
            "Acréscimos acumulados (R$)": round(acumulado_acrescimos, 2), "Supressões acumuladas (R$)": round(acumulado_supressoes, 2),
            "Saldo para novos acréscimos (R$)": round(saldo_acrescimo, 2),
            "% acréscimos acumulados": percentual_acrescimos, "Status": status, "Observação": row.get("Observação", ""),
        })
...
def gerar_excel(resultado, eventos, parametros):
    ...
    with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
        ...
        fmt_header = workbook.add_format({"bold": True, "bg_color": "#1F4E79", "font_color": "#FFFFFF", "border": 1, "align": "center", "valign": "vcenter"})
        ...
        colunas_pct_col = [c for c in eventos.columns if "%" in c]
        for c in colunas_moeda:
            idx = eventos.columns.get_loc(c)
            ws_e.set_column(idx, idx, 22, fmt_money)
        for c in colunas_pct_col:
            idx = eventos.columns.get_loc(c)
            ws_e.set_column(idx, idx, 18, fmt_pct)
        if "Status" in eventos.columns:
            idx = eventos.columns.get_loc("Status")
            ws_e.conditional_format(1, idx, max(len(eventos), 1), idx, {"type": "text", "criteria": "containing", "value": "OK", "format": fmt_ok})
            ws_e.conditional_format(1, idx, max(len(eventos), 1), idx, {"type": "text", "criteria": "containing", "value": "EXTRAPOLA", "format": fmt_bad})
    buffer.seek(0)
...
st.info("Este módulo avalia a aderência de aditivos e supressões ao limite percentual aplicável. Ele é independente do Valor Global.")

with st.expander("Parâmetros do contrato", expanded=True):
    col1, col2 = st.columns(2)
    with col1:
        valor_original_txt = st.text_input("Valor original do contrato (R$)", value="", placeholder="Ex.: 167.770,65")
...
st.caption("Nos campos de valor, digite no padrão brasileiro: 1.234,56. No percentual: 0,99 = 0,99%; 10 = 10%.")
...
eventos_input = st.data_editor(
    ...
    column_config={
        ...
        "Tipo do evento": st.column_config.SelectboxColumn("Tipo do evento",
            options=["", "Acréscimo", "Supressão", "Restabelecimento do mesmo item",
                     "Reajuste/revisão/reequilíbrio", "Informativo", "Outro"]),
        "% sobre base legal": st.column_config.TextColumn("% sobre base legal"),
        "Valor informado (R$)": st.column_config.TextColumn("Valor informado (R$)"),
        "Observação": st.column_config.TextColumn("Observação"),
    },
    num_rows="dynamic", key="avaliacao_aditivos_editor",
)
...
status_bg = "#FCE4D6" if status_geral == "EXTRAPOLA" else "#DBEAFE"
status_fg = "#7F1D1D" if status_geral == "EXTRAPOLA" else "#1E3A8A"
status_border = "#F4B183" if status_geral == "EXTRAPOLA" else "#93C5FD"
c7.markdown(
    f"""<div style="border:1px solid {status_border}; background:{status_bg}; color:{status_fg};
        ...
        <div style="font-size:1.45rem; font-weight:800;">{status_geral}</div></div>""",
    unsafe_allow_html=True,
)

if status_geral == "EXTRAPOLA":
    st.error("Os acréscimos acumulados ultrapassam o limite informado.")
else:
...
st.download_button("Baixar Aditivos: 25% em XLSX", data=excel_bytes,
    file_name="Aditivos_25.xlsx",
    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", type="primary")
st.caption("Observação: o módulo é apoio de governança. A aplicação do limite deve ser validada conforme o caso concreto.")

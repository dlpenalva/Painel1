...
def aplicar_css_responsivo_relatorio():
    st.markdown(
        ...
        .cl8us-kpi-destaque-label {
            font-size:0.92rem;
            color:#27496D;
            font-weight:600;
        }
        .cl8us-kpi-destaque-valor {
...
def _percentual_ciclo_minuta(ciclo):
    for chave in ["percentual_aplicado", "Percentual aplicado", "Variação", "variacao", "var"]:
        if chave in ciclo and ciclo.get(chave) not in [None, ""]:
            try:
                return percentual(float(ciclo.get(chave)), 4 if chave in ["percentual_aplicado", "Percentual aplicado"] else 2)
            except Exception:
...
def gerar_minuta_apostilamento_docx(adm, res):
    ...
    considerandos = [
        ...
        f"A memória de cálculo constante em [campo a preencher], que apurou o reajuste acumulado de {percentual(res.get('variacao_acumulada', res.get('fator_acumulado', 1.0) - 1), 4)};",
    ...
    if ciclos_validos:
        ...
        _adicionar_subitem(
            ...
            f"O percentual acumulado apurado corresponde a {percentual(res.get('variacao_acumulada', res.get('fator_acumulado', 1.0) - 1), 4)}."
        )
    else:
    ...
    _adicionar_item_numerado(document, 4, "Para fins de consolidação, registra-se:")
    _adicionar_subitem(
        document,
        "a)",
        f"Valor global atualizado do contrato: de {moeda(res.get('valor_original_contrato', 0))} para {moeda(res.get('valor_atualizado_contrato', res.get('valor_global_estoque', 0)))}."
    )
    _adicionar_subitem(
        document,
        "b)",
        f"Valor consolidado da execução apurada por ciclos: {moeda(res.get('valor_executado_atualizado', res.get('total_devido_reajustado', 0)))}."
    )
...
st.markdown(
    f"""
    <div class="cl8us-kpi-destaque">
        <div class="cl8us-kpi-destaque-label">Valor atualizado do contrato</div>
        <div class="cl8us-kpi-destaque-valor">{moeda(res.get("valor_atualizado_contrato", res.get("valor_global_estoque", 0)))}</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
col4.metric("Delta total", moeda(res.get("delta_acumulado", 0)))

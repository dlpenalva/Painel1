...
def criar_pdf_relatorio(adm, res):
    ...
    indicadores = [
        ...
        ["Valor represado a pagar", moeda(res.get("valor_represado_a_pagar", res.get("delta_total", res.get("delta_acumulado", 0))))],
        ["Saldo remanescente atualizado", moeda(res.get("remanescente_reajustado", 0))],
...
def tabela_dataframe_pdf(df):
    ...
    if df is None or not isinstance(df, pd.DataFrame) or df.empty:
        return tabela_pdf([["Informação", "Sem dados disponíveis"]], col_widths=[5*cm, 10*cm])
    ...
    dados = [[Paragraph(_limpar_celula_pdf(c), header_style) for c in df.columns]]
    for _, row in df.iterrows():
        dados.append([Paragraph(_limpar_celula_pdf(v), cell_style) for v in row.tolist()])

    largura_total = 18.2 * cm
...
col4.metric("Valor Represado a Pagar", moeda(res.get("valor_represado_a_pagar", res.get("delta_total", res.get("delta_acumulado", 0)))))
